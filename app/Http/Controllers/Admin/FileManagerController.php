...
class FileManagerController extends Controller
{
    public function store_file(Request $request)
    {
        if ($request->hasFile('fm_file')) {
            ...
            // });
            // $canvas->insert($image);
            // $canvas->insert(interImage::make(public_path('logo.png')), 'bottom-right');
            ...
            // book size
            $canvas = interImage::canvas(350, 500);
            $image->resize(350, 503, function ($constraint) {
                $constraint->aspectRatio();
            });
            $canvas->insert($image,'center-center');
            $canvas->insert(interImage::make(public_path('logo.png'))->opacity(50), 'bottom-right');

            $path = 'uploads/file_manager/fm_image_350x500_' . $temp_name . '.' . $extension;
            $canvas->save($path);
            $this->image_save_to_db($path);
    ...
    {
        $images = Image::latest()->where('status', 1)->paginate(12);
        return $images;
